...
const changeToBlack = event => {
    sayHelloBtn.style.backgroundColor = `black`;
    sayHelloBtn.style.color = `white`;
}

sayHelloBtn.addEventListener(`mouseover`, changeToBlack)

// PROBLEM 3
/*
Now you can see that the button colors change, but they do not change back when we take the mouse off of the button.

Write another function that changes the button back to its original colors.  for the background and black for the text.
...
const changeBack = event => {
    sayHelloBtn.style.backgroundColor = `#EFEFEF`;
    sayHelloBtn.style.color = `black`;
}

sayHelloBtn.addEventListener(`mouseout`, changeBack)
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/Is This Fun Yet?').then((res) => {
...
const test = event => {
    axios.get('http://localhost:3000/query-test?myquery=sadness').then((res) => {
        console.log(res.data)
    })
}
...
/*
    ...
    2: Send more than 1 query on the URL -- what happened? 
        turns other query into value of first?
*/
...
// function createList(food) {
    ...
    const createFood = event => {
        ...
        axios.post(`http://localhost:3000/food`,body).then((res) => {
            ...
            item.textContent = res.data[res.data.length - 1]
            // console.log(res.data)
            document.body.appendChild(item)
    })
}

document.getElementById(`daFood`).addEventListener(`submit`,createFood)